...
use super::{
	Order, OrderFees, OrderMetadata, OrderPriority, OrderQuoteData, OrderValidationError,
...
pub struct OrdersRequest {
	...
	/// Deadline for order execution (Unix timestamp)
...
/// Fee preferences for order execution
#[derive(Debug, Clone, Deserialize, Serialize)]
#[cfg_attr(feature = "openapi", derive(ToSchema))]
...
/// Execution preferences for order processing
#[derive(Debug, Clone, Deserialize, Serialize)]
#[cfg_attr(feature = "openapi", derive(ToSchema))]
...
impl OrdersRequest {
	...
	pub fn validate(&self) -> OrderValidationResult<()> {
		...
		if !is_valid_ethereum_address(&self.user_address) {
			return Err(OrderValidationError::InvalidUserAddress {
		...
		match (&self.quote_id, &self.quote_response) {
			(Some(_), Some(_)) => {
				return Err(OrderValidationError::ConflictingQuoteData);
			},
			(None, None) => {
				return Err(OrderValidationError::MissingQuoteData);
			},
			_ => {},
		}
		...
		if let Some(ref quote_id) = self.quote_id {
			if quote_id.is_empty() {
				return Err(OrderValidationError::InvalidQuoteId {
		...
		if let Some(slippage) = self.slippage_tolerance {
			if !(0.0..=1.0).contains(&slippage) {
				return Err(OrderValidationError::InvalidSlippage { slippage });
			}
		}
		...
		if let Some(deadline) = self.deadline {
			let deadline_dt = DateTime::from_timestamp(deadline, 0).ok_or_else(|| {
				OrderValidationError::InvalidDeadline {
					reason: "Invalid timestamp format".to_string(),
				}
			})?;

			if deadline_dt <= Utc::now() {
				return Err(OrderValidationError::DeadlineInPast);
			}

			if deadline_dt > Utc::now() + Duration::hours(24) {
				return Err(OrderValidationError::DeadlineTooFar { max_hours: 24 });
			}
		}
		...
		if let Some(ref signature) = self.signature {
			if !signature.starts_with("0x") || signature.len() != 132 {
				return Err(OrderValidationError::InvalidSignature {
	...
	pub fn to_domain(&self, ip_address: Option<String>) -> OrderValidationResult<Order> {
		// Validate first
		self.validate()?;
		...
		if let Some(ref quote_id) = self.quote_id {
			order = order.with_quote_id(quote_id.clone());
		}

		if let Some(ref quote_response) = self.quote_response {
			let quote_data = quote_response.to_domain()?;
			order = order.with_quote_data(quote_data);
		}

		// Set signature
		if let Some(ref signature) = self.signature {
			order = order.with_signature(signature.clone());
		}

		// Create metadata
		let mut metadata = OrderMetadata::new("api".to_string());
		...
		order = order.with_metadata(metadata);
		...
		if let Some(ref exec_prefs) = self.execution_preferences {
			if let Some(ref priority_str) = exec_prefs.priority {
				...
				order = order.with_priority(priority);
			}
		}

		// Set fees
		let mut fees = OrderFees::default();
		if let Some(ref fee_prefs) = self.fee_preferences {
...
impl QuoteResponseRequest {
	...
	pub fn validate(&self) -> OrderValidationResult<()> {
		...
		if !is_valid_ethereum_address(&self.token_in) {
			return Err(OrderValidationError::UnsupportedToken {
		...
		self.amount_in
			.parse::<u64>()
			.map_err(|_| OrderValidationError::InvalidConfiguration {
		...
		if !is_valid_chain_id(self.chain_id) {
			return Err(OrderValidationError::UnsupportedChain {
				chain_id: self.chain_id,
			});
		}
		...
		if let Some(price_impact) = self.price_impact {
			if !(0.0..=1.0).contains(&price_impact) {
				return Err(OrderValidationError::InvalidConfiguration {
		...
		if let Some(expires_at) = self.expires_at {
			...
			if expires_dt <= Utc::now() {
				return Err(OrderValidationError::QuoteExpired {
					quote_id: self
						.solver_quote_id
						.clone()
	...
	pub fn to_domain(&self) -> OrderValidationResult<OrderQuoteData> {
		self.validate()?;
		...
		let mut quote_data = OrderQuoteData::new(
			self.token_in.clone(),
...
impl FeePreferences {
	pub fn validate(&self) -> OrderValidationResult<()> {
		if let Some(max_fee_rate) = self.max_platform_fee_rate {
			if !(0.0..=0.1).contains(&max_fee_rate) {
				return Err(OrderValidationError::InvalidConfiguration {
...
mod tests {
	...
	fn test_valid_intent_request() {
		...
		let order = request.to_domain(Some("192.168.1.1".to_string())).unwrap();
		assert_eq!(
			order.user_address,
	...
	fn test_missing_quote_data() {
		...
		assert!(matches!(
			request.validate(),
			Err(OrderValidationError::MissingQuoteData)
		));
	}
	...
	fn test_conflicting_quote_data() {
		...
		assert!(matches!(
			request.validate(),
			Err(OrderValidationError::ConflictingQuoteData)
		));
	}
	...
	fn test_invalid_slippage() {
		...
		assert!(matches!(
			request.validate(),
			Err(OrderValidationError::InvalidSlippage { slippage: 1.5 })
		));
	}
	...
	fn test_deadline_in_past() {
		...
		assert!(matches!(
			request.validate(),
			Err(OrderValidationError::DeadlineInPast)
		));
	}
	...
	fn test_deadline_too_far() {
		...
		assert!(matches!(
			request.validate(),
			Err(OrderValidationError::DeadlineTooFar { max_hours: 24 })
		));
	}
	...
	fn test_expired_quote() {
		...
		assert!(matches!(
			quote_response.validate(),
			Err(OrderValidationError::QuoteExpired { .. })
		));
	}
	...
	fn test_request_with_quote_response() {
		...
		let order = request.to_domain(None).unwrap();
		assert!(order.quote_data.is_some());
	...
	fn test_request_with_client_info() {
		...
		let order = request.to_domain(Some("192.168.1.1".to_string())).unwrap();
		assert!(order.metadata.custom_data.contains_key("app_name"));
		assert_eq!(order.metadata.ip_address, Some("192.168.1.1".to_string()));
	}
}
