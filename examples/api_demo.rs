...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Submit intent (stateless)
    println!("\n3. Submit Intent (Stateless)");
    ...
    let intent_response = client
        .post(&format!("{}/v1/intents", base_url))
        .json(&intent_request)
        .send()
        .await?;

    println!("   Status: {}", intent_response.status());
    if intent_response.status().is_success() {
        let intent_json: serde_json::Value = intent_response.json().await?;
        let intent_id = intent_json["intent_id"].as_str().unwrap();
        println!("   Intent ID: {}", intent_id);
        println!("   Status: {}", intent_json["status"]);

        // Query intent status
        println!("\n4. Query Intent Status");
        let status_response = client
            .get(&format!("{}/v1/intents/{}", base_url, intent_id))
            .send()
            .await?;
        ...
        if status_response.status().is_success() {
            ...
            println!("   Intent Status: {}", status_json["status"]);
