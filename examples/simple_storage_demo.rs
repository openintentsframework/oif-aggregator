...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
	...
	let (_solvers1, total1, _, _) = state1
		.solver_service
	...
	let (_solvers2, total2, _, _) = state2
		.solver_service
		.list_solvers_paginated(None, None)
		.await?;
	...
	let (_solvers3, total3, _, _) = state3
		.solver_service
		.list_solvers_paginated(None, None)
		.await?;
	println!(
		"   ✅ Pre-populated storage: {} solvers (1 pre-existing + 1 added)",
		total3
	);
	...
	// List all solvers
	let (solvers, total_count, active_count, healthy_count) = state3
		.solver_service
		.list_solvers_paginated(None, None)
		.await?;
	...
	for solver in &solvers {
		println!(
			"      - {}: {} ({})",
			solver.solver_id, solver.adapter_id, solver.endpoint
		);
	}
	...
	println!(
		"   📂 Direct storage query: {} solvers found",
		all_solvers.len()
	);
	...
	println!(
		"      - Count query: {} solvers in {:?}",
		solver_count, duration
	);
