...
use oif_aggregator::mocks::{MockDemoAdapter, MockTestAdapter, mock_solver};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (_app1, _state1) = AggregatorBuilder::default()
        .start()
        .await?;
    ...
    let (solvers, total_count, active_count, _) = state3.solver_service.list_solvers_paginated(None, None).await?;
    println!("   📊 Registered solvers: {} total, {} active", total_count, active_count);
    for solver in solvers.iter().take(5) { // Show first 5
        println!("      - {} (adapter: {})", solver.solver_id, solver.adapter_id);
    }
