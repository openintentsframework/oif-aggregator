...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
	...
	let solver_builder = AggregatorBuilder::new().with_solver(custom_solver).await;
	...
	let config_builder = AggregatorBuilder::from_config(Settings::default()).await;
	...
	let chained_builder = AggregatorBuilder::new();
