...
use oif_aggregator::{AggregatorBuilder, Solver, config::Settings, storage::MemoryStore};
...
async fn test_builder_default() {
    let builder = AggregatorBuilder::default();
    assert!(builder.settings().is_none());
}
...
async fn test_builder_from_config() {
    ...
    let builder = AggregatorBuilder::from_config(settings.clone());
...
async fn test_builder_with_solver() {
    ...
    let builder = AggregatorBuilder::new().with_solver(solver.clone());
...
async fn test_builder_with_storage() {
    ...
    let builder = AggregatorBuilder::new().with_storage(custom_storage);
...
#[tokio::test]
async fn test_builder_with_ttl_cleanup() {
    let solver = create_test_solver();
    let builder = AggregatorBuilder::new()
        .with_solver(solver)
        .with_ttl_cleanup(true);

    // Test that TTL cleanup doesn't break the builder chain
    let result = builder.start().await;
    assert!(result.is_ok());
}
...
async fn test_builder_with_config_and_solver() {
    ...
    let builder = AggregatorBuilder::from_config(settings).with_solver(solver);
