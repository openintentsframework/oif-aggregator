//! Solver options E2E tests
//!
//! These tests verify solver selection strategies, timeout handling,
//! and configuration validation in real API scenarios.
...
mod solver_options_e2e {
	...
	async fn test_solver_options_default_behavior() {
		...
		assert_eq!(metadata["globalTimeoutMs"].as_u64().unwrap(), 10000); // Default 10s
		assert_eq!(metadata["solverTimeoutMs"].as_u64().unwrap(), 5000); // Default 5s
	...
	async fn test_solver_options_global_timeout_smaller_than_per_solver() {
		...
		assert!(message.contains(
			"Global timeout (1000ms) should not be less than per-solver timeout (2000ms)"
		));
	...
	async fn test_solver_options_include_solvers() {
		...
		assert_eq!(
			...
			1,
			"Exactly 1 solver should have been contacted"
		);
	...
	async fn test_solver_options_solver_selection_all() {
		...
		if resp.status().is_success() {
			...
			assert_eq!(
				...
				"'all' strategy should query all available solvers"
			);
		} else {
	...
	async fn test_solver_options_solver_selection_sampled() {
		...
		if resp.status().is_success() {
			...
			assert!(
				queried <= 5,
				"Sampled strategy should not query more than sampleSize (5)"
			);
			assert!(
				queried <= total_available,
				"Can't query more than available"
			);
		} else {
			// Response not successful in test environment (acceptable with mock solvers)
		}
	...
	async fn test_solver_options_solver_selection_priority() {
		...
		if status.is_success() {
			...
			assert_eq!(
				metadata["solverSelectionMode"].as_str().unwrap(),
				"priority"
			);
	...
	async fn test_solver_options_min_quotes_early_termination() {
		...
		if resp.status().is_success() {
			...
			assert!(
				total_duration < 8000,
				"Should complete within 8s even without early termination"
			);
		} else {
			// Response not successful in test environment (acceptable with mock solvers)
			// Still verify timing
	...
	async fn test_solver_options_complex_scenario() {
		...
		if resp.status().is_success() {
			...
			assert_eq!(
				success + error + timeout,
				1,
				"Should have exactly 1 solver response"
			);
		} else {
			// Response not successful in test environment (acceptable with mock solvers)
		}
	...
	async fn test_solver_options_minimum_boundary_timeout_values() {
		...
		assert!(
			total_duration <= 500,
			"Should complete quickly with 100ms timeouts, took {}ms",
			total_duration
		);
	...
	async fn test_metadata_solver_response_distribution() {
		...
		assert_eq!(
			success + error + timeout,
			queried,
			"Response counts must sum to queried count"
		);
	...
	async fn test_metadata_timing_accuracy() {
		...
		if resp.status().is_success() {
			...
			assert!(
				diff < 1000,
				"Timing should be accurate within 1s: reported {}ms, wall clock {}ms",
				reported_duration,
				wall_clock_ms
			);
			...
			assert!(
				reported_duration <= timeout_ms + 100, // Allow small overhead
	...
	async fn test_solver_inclusion_filtering_behavior() {
		...
		if resp.status().is_success() {
			...
			assert_eq!(
				metadata["solversQueried"].as_u64().unwrap(),
				1,
				"Should query exactly 1 solver when including only fast-solver"
			);
			assert!(
				metadata["totalSolversAvailable"].as_u64().unwrap() >= 4,
			...
			if !quotes.is_empty() {
				...
				assert!(
					success >= 1,
					"Should have at least 1 successful response when quotes are returned"
				);
				...
				for quote in quotes {
					...
					assert_eq!(
						solver_id, "fast-solver",
						"All quotes should come from fast-solver, but got quote from: {}",
						solver_id
					);
					...
					if let Some(adapter_id) = quote["orders"][0]["message"]["adapter"].as_str() {
						assert_eq!(
							adapter_id, "timing-fast",
							"Adapter ID should be timing-fast, but got: {}",
							adapter_id
						);
					}
				}
			...
			assert!(
				fast_adapter.call_count() > 0,
				"Fast adapter should have been called (matches metadata)"
			);
			assert_eq!(
				slow_adapter.call_count(),
				0,
				"Slow adapter should NOT have been called (matches metadata)"
			);
		} else {
			// If request failed, still verify call behavior for debugging
			assert!(
				fast_adapter.call_count() > 0,
				"Fast adapter should have been called even if request failed"
			);
			assert_eq!(
				slow_adapter.call_count(),
				0,
				"Slow adapter should NOT have been called"
			);
		}
		...
		if resp.status().is_success() {
			...
			if !quotes.is_empty() {
				...
				for quote in quotes {
					let solver_id = quote["solverId"].as_str().unwrap_or("unknown");
					assert!(
						!excluded_solvers.contains(&solver_id),
						"Found quote from excluded solver: {}. Excluded: {:?}",
						solver_id,
						excluded_solvers
					);
				}
			...
			// Secondary validation: Verify adapter call behavior matches metadata
			assert!(
				fast_adapter.call_count() > 0 || slow_adapter.call_count() > 0,
				"At least one of fast/slow adapters should have been called (matches metadata)"
			);
			assert_eq!(
				timeout_adapter.call_count(),
				0,
	...
	async fn test_timeout_behavior_verification() {
		...
		if resp.status().is_success() {
			...
			assert_eq!(
				metadata["solverTimeoutMs"].as_u64().unwrap(),
				500,
			...
			assert!(
				queried >= 2,
				"Should have queried at least 2 solvers (fast and slow)"
			);
			// In test environments, not all queried solvers may respond within the test timeframe
			...
			assert!(
				success >= 1,
				"Fast solver should have succeeded within 500ms timeout"
			);
			...
			assert!(
				slow_adapter.call_count() > 0,
				"Slow adapter should have been called even if timed out (matches metadata)"
			);
		} else {
			...
			assert!(
				fast_adapter.call_count() > 0,
				"Fast adapter should have been called"
			);
			assert!(
				slow_adapter.call_count() > 0,
				"Slow adapter should have been called (even if timed out)"
			);
		}
	...
	async fn test_early_termination_with_min_quotes() {
		...
		if resp.status().is_success() {
			...
			assert_eq!(
				metadata["minQuotesRequired"].as_u64().unwrap(),
				1,
			...
			if !quotes.is_empty() && success >= 1 {
				...
				assert!(
					queried <= total_available,
					"Queried {} solvers, but only {} available",
					queried,
					total_available
				);
	...
	async fn test_global_timeout_enforcement() {
		...
		assert!(
			...
			"Global timeout should cut off request quickly, took {}ms",
			elapsed.as_millis()
		);
		...
		if resp.status().is_success() {
			...
			// Verify timeout configuration was applied correctly
			assert_eq!(
				metadata["globalTimeoutMs"].as_u64().unwrap(),
				1000,
			...
			let duration = metadata["totalDurationMs"].as_u64().unwrap();
			assert!(
				duration <= 1100, // Allow small overhead
				"Duration should not exceed global timeout significantly: {}ms",
				duration
			);
			...
			if let Ok(body_text) = resp.text().await {
				if let Ok(body_json) = serde_json::from_str::<serde_json::Value>(&body_text) {
					if let Some(error) = body_json.get("error") {
						...
						assert!(
							error.as_str().unwrap_or("").contains("AGGREGATION")
								|| error.as_str().unwrap_or("").contains("TIMEOUT"),
							"Expected timeout or aggregation error, got: {}",
							error
						);
					}
				}
				...
				if let Some(error) = body_json.get("error") {
					...
					if let Some(quotes) = body_json["quotes"].as_array() {
						if !quotes.is_empty() {
							...
							let total_calls = fast_adapter.call_count()
								+ slow_adapter.call_count()
								+ failing_adapter.call_count();
	...
	async fn test_solver_selection_strategy_metadata() {
		...
		if resp.status().is_success() {
			...
			assert!(
				success + error + timeout <= 10,
				"Response counts seem unreasonable"
			);
			assert!(
				success + error + timeout >= 1,
	...
	async fn test_comprehensive_timing_verification() {
		...
		let mut fast_request = ApiFixtures::valid_quote_request();
		...
		let mut mixed_request = ApiFixtures::valid_quote_request();
		...
		if fast_resp.status().is_success() {
			...
			assert_eq!(
				fast_metadata["solversQueried"].as_u64().unwrap(),
				1,
			...
			assert!(
				fast_duration < 500,
				"Fast-only scenario should complete very quickly: {}ms",
				fast_duration
			);
			...
			if let Some(fast_quotes) = fast_body["quotes"].as_array() {
				if !fast_quotes.is_empty() {
					for quote in fast_quotes {
						...
						assert_eq!(
							solver_id, "fast-solver",
							"Fast-only scenario should only have quotes from fast-solver, got: {}",
							solver_id
						);
					}
				}
		...
		if mixed_resp.status().is_success() {
			...
			assert_eq!(
				mixed_metadata["solversQueried"].as_u64().unwrap(),
				2,
			...
			assert_eq!(
				mixed_metadata["minQuotesRequired"].as_u64().unwrap(),
				2,
			...
			if !early_termination {
				...
				assert!(
				mixed_duration > 1000,
				"Mixed scenario without early termination should take time for slow solver: {}ms",
				mixed_duration
			);
			}
			...
			if let Some(mixed_quotes) = mixed_body["quotes"].as_array() {
				if !mixed_quotes.is_empty() {
					...
					for quote in mixed_quotes {
						...
						assert!(
						allowed_solvers.contains(&solver_id),
						"Mixed scenario should only have quotes from fast/slow solvers, got: {}",
						solver_id
					);
					}
				}
		...
		assert!(
			fast_adapter.call_count() >= 2,
			"Fast adapter should have been called in both scenarios (matches metadata)"
		);
		assert!(
			slow_adapter.call_count() >= 1,
			"Slow adapter should have been called in mixed scenario (matches metadata)"
		);
