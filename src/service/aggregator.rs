...
impl AggregatorService {
    ...
    pub async fn initialize_adapters(&mut self) -> Result<(), AdapterError> {
        ...
        for (solver_id, solver) in &self.solvers {
            // Create adapter configuration from solver
            let adapter_config = AdapterConfig {
                adapter_id: solver.adapter_id.clone(),
                adapter_type: AdapterType::OifV1, // Default for now, should be configurable
                name: solver
                    .metadata
                    .name
                    .clone()
                ...
                supported_chains: solver.metadata.supported_chains.clone(), // No need to parse, already u64
                configuration: {
                    let mut config = serde_json::Map::new();
                    config.insert(
                        "endpoint".to_string(),
                        serde_json::Value::String(solver.endpoint.clone()),
                    );
                    config.insert(
                        "timeout_ms".to_string(),
                        serde_json::Value::Number(serde_json::Number::from(solver.timeout_ms)),
                    );
                    if let Some(headers) = &solver.metadata.headers {
                        config.insert("headers".to_string(), serde_json::to_value(headers)?);
                    }
                    serde_json::Value::Object(config)
                },
                enabled: true,
                created_at: solver.created_at,
                timeout_ms: Some(solver.timeout_ms),
                max_retries: Some(solver.metadata.max_retries),
                rate_limit: None, // TODO: Configure rate limiting
                headers: solver.metadata.headers.clone(),
                required_capabilities: vec!["quotes".to_string()], // Default capability
                optional_capabilities: vec!["intents".to_string()], // Default optional capability
            };

            match AdapterFactory::create_from_config(&adapter_config) {
                Ok(adapter) => {
                    info!(
