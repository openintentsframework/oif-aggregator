//! Intent response models for API layer
...
use super::{
    Intent, IntentExecutionResult, IntentFees, IntentPriority, IntentStatus, IntentSystemHealth,
};

/// Response body for /v1/intents endpoint (intent submission)
...
pub struct IntentsResponse {
    ...
    /// Fee information
    pub fees: IntentFeesResponse,

    /// Next steps for the user
    pub next_steps: Vec<String>,
}

/// Response body for /v1/intents/{id} endpoint (intent status)
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct IntentStatusResponse {
    ...
    /// Execution history
    pub execution_history: Vec<IntentExecutionResponse>,

    /// Progress information
    pub progress: IntentProgressResponse,

    /// Fee breakdown
    pub fees: IntentFeesResponse,

    /// Metadata (filtered for public API)
    pub metadata: IntentMetadataResponse,
}

/// Intent execution result in API format
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Intent result API format (alias for backward compatibility)
pub type IntentResultApi = IntentResultResponse;

/// Intent execution attempt in API format
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct IntentExecutionResponse {
...
/// Intent progress information
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct IntentProgressResponse {
    ...
    /// Whether the intent can be cancelled
    pub can_cancel: bool,

    /// Whether the intent can be retried
    pub can_retry: bool,
...
/// Intent metadata in API format (filtered for public consumption)
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct IntentMetadataResponse {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct IntentsCollectionResponse {
    /// List of intents
    pub intents: Vec<IntentStatusResponse>,
    ...
    /// Summary statistics
    pub statistics: IntentStatisticsResponse,
...
/// Intent statistics for monitoring
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct IntentStatisticsResponse {
...
/// System health for intent processing
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Detailed intent analysis for admin endpoints
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct IntentAnalysisResponse {
    ...
    /// Performance metrics
    pub performance: IntentPerformanceResponse,

    /// Risk assessment
    pub risk_assessment: IntentRiskResponse,

    /// Profitability analysis
    pub profitability: IntentProfitabilityResponse,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct IntentPerformanceResponse {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct IntentProfitabilityResponse {
...
impl IntentsResponse {
    /// Create response from domain intent
    pub fn from_domain(intent: &Intent) -> IntentResult<Self> {
        Ok(Self {
            intent_id: intent.intent_id.clone(),
            status: intent.status.to_string(),
            message: Self::generate_status_message(&intent.status, intent.priority),
            timestamp: Utc::now().timestamp(),
            estimated_execution_time_ms: intent.estimated_execution_time_ms,
            priority: intent.priority.to_string(),
            quote_id: intent.quote_id.clone(),
            fees: IntentFeesResponse::from_domain(&intent.fees),
            next_steps: Self::generate_next_steps(&intent.status),
        })
    }

    fn generate_status_message(status: &IntentStatus, priority: IntentPriority) -> String {
        match status {
            IntentStatus::Pending => "Intent received and is being validated".to_string(),
            IntentStatus::Submitted => "Intent has been submitted to solver".to_string(),
            IntentStatus::Queued => {
                format!("Intent queued for execution with {} priority", priority)
            }
            IntentStatus::Executing => "Intent is currently being executed".to_string(),
            IntentStatus::Success => "Intent executed successfully".to_string(),
            IntentStatus::Failed => "Intent execution failed".to_string(),
            IntentStatus::Cancelled => "Intent was cancelled".to_string(),
            IntentStatus::Expired => "Intent expired before execution".to_string(),
            IntentStatus::Simulating => "Intent is being simulated".to_string(),
            IntentStatus::ReviewRequired => "Intent requires manual review".to_string(),
        }
    }

    fn generate_next_steps(status: &IntentStatus) -> Vec<String> {
        match status {
            IntentStatus::Pending => vec![
                "Monitor intent status for updates".to_string(),
                "Ensure sufficient balance in wallet".to_string(),
            ],
            IntentStatus::Submitted => vec![
                "Intent is being processed by solver".to_string(),
                "Monitor for execution confirmation".to_string(),
            ],
            IntentStatus::Queued => vec![
                "Intent will be executed automatically".to_string(),
                "Monitor for transaction hash".to_string(),
            ],
            IntentStatus::Executing => vec![
                "Transaction is being processed".to_string(),
                "Wait for confirmation".to_string(),
            ],
            IntentStatus::Success => vec![
                "Check your wallet for received tokens".to_string(),
                "View transaction on block explorer".to_string(),
            ],
            IntentStatus::Failed => vec![
                "Check error details".to_string(),
                "Consider submitting a new intent".to_string(),
            ],
            IntentStatus::Cancelled => vec![
                "Intent processing stopped".to_string(),
                "Submit a new intent if needed".to_string(),
            ],
            IntentStatus::Expired => vec!["Submit a new intent with fresh quote".to_string()],
            IntentStatus::Simulating => vec![
                "Simulation in progress".to_string(),
                "Results will determine execution".to_string(),
            ],
            IntentStatus::ReviewRequired => vec![
...
impl IntentStatusResponse {
    /// Create response from domain intent
    pub fn from_domain(intent: &Intent) -> IntentResult<Self> {
        let latest_result = intent.latest_execution_result();
        ...
        Ok(Self {
            ...
            result: latest_result.map(IntentResultResponse::from_domain),
            execution_history: intent
                .execution_history
                .iter()
                .map(IntentExecutionResponse::from_domain)
                .collect(),
            progress: IntentProgressResponse::from_domain(intent),
            fees: IntentFeesResponse::from_domain(&intent.fees),
            metadata: IntentMetadataResponse::from_domain(&intent.metadata),
        })
    }

    /// Create minimal response for public API
    pub fn minimal_from_domain(intent: &Intent) -> IntentResult<Self> {
        let mut response = Self::from_domain(intent)?;
...
impl IntentProgressResponse {
    fn from_domain(intent: &Intent) -> Self {
        let progress_percent = match intent.status {
            IntentStatus::Pending => 10,
            IntentStatus::Submitted => 20,
            IntentStatus::Queued => 25,
            IntentStatus::Simulating => 40,
            IntentStatus::Executing => 75,
            IntentStatus::Success => 100,
            IntentStatus::Failed | IntentStatus::Cancelled | IntentStatus::Expired => 0,
            IntentStatus::ReviewRequired => 50,
        };

        let current_step = match intent.status {
            IntentStatus::Pending => "Validating intent",
            IntentStatus::Submitted => "Submitted to solver",
            IntentStatus::Queued => "Waiting for execution",
            IntentStatus::Simulating => "Simulating transaction",
            IntentStatus::Executing => "Executing transaction",
            IntentStatus::Success => "Completed successfully",
            IntentStatus::Failed => "Execution failed",
            IntentStatus::Cancelled => "Cancelled",
            IntentStatus::Expired => "Expired",
            IntentStatus::ReviewRequired => "Under review",
        }
        .to_string();
        ...
        Self {
            progress_percent,
            current_step,
            estimated_time_remaining_secs: intent.estimated_execution_time_ms.map(|ms| ms / 1000),
            can_cancel: !intent.is_final_state(),
            can_retry: intent.can_retry(),
            seconds_until_deadline: intent.seconds_until_deadline(),
            retry_count: intent.retry_count,
            max_retries: intent.priority.retry_count(),
        }
    }
}

impl IntentFeesResponse {
...
impl IntentMetadataResponse {
    fn from_domain(metadata: &super::IntentMetadata) -> Self {
...
impl SystemHealthResponse {
    pub fn from_domain(health: &IntentSystemHealth) -> Self {
        Self {
            ...
            pending_intents: health.pending_intents,
            executing_intents: health.executing_intents,
...
impl ToString for IntentStatus {
    fn to_string(&self) -> String {
        match self {
            IntentStatus::Pending => "pending".to_string(),
            IntentStatus::Submitted => "submitted".to_string(),
            IntentStatus::Queued => "queued".to_string(),
            IntentStatus::Executing => "executing".to_string(),
            IntentStatus::Success => "success".to_string(),
            IntentStatus::Failed => "failed".to_string(),
            IntentStatus::Cancelled => "cancelled".to_string(),
            IntentStatus::Expired => "expired".to_string(),
            IntentStatus::Simulating => "simulating".to_string(),
            IntentStatus::ReviewRequired => "review_required".to_string(),
        }
    }
}

/// Convert domain Intent to API response
impl TryFrom<Intent> for IntentsResponse {
    type Error = super::IntentError;

    fn try_from(intent: Intent) -> Result<Self, Self::Error> {
        IntentsResponse::from_domain(&intent)
    }
}

/// Convert domain Intent to status response
impl TryFrom<Intent> for IntentStatusResponse {
    type Error = super::IntentError;

    fn try_from(intent: Intent) -> Result<Self, Self::Error> {
        IntentStatusResponse::from_domain(&intent)
    }
}
...
mod tests {
    ...
    #[test]
    fn test_intents_response_from_domain() {
        let intent = create_test_intent();
        let response = IntentsResponse::from_domain(&intent).unwrap();

        assert_eq!(response.intent_id, intent.intent_id);
    ...
    #[test]
    fn test_intent_status_response_from_domain() {
        let mut intent = create_test_intent();

        // Add execution result
        let execution_result = super::IntentExecutionResult::success(
            "0x123".to_string(),
            12345,
            21000,
            "1000000".to_string(),
        ...
        intent.add_execution_result(execution_result);

        let response = IntentStatusResponse::from_domain(&intent).unwrap();
    ...
    fn test_minimal_status_response() {
        let intent = create_test_intent();
        let response = IntentStatusResponse::minimal_from_domain(&intent).unwrap();

        // User address should be truncated
        assert!(response.user_address.contains("..."));
        assert!(response.user_address.len() < intent.user_address.len());
    ...
    fn test_progress_response() {
        ...
        let progress = IntentProgressResponse::from_domain(&intent);
        assert_eq!(progress.progress_percent, 10); // Pending
        assert_eq!(progress.current_step, "Validating intent");

        intent.update_status(super::IntentStatus::Executing);
        let progress = IntentProgressResponse::from_domain(&intent);
        assert_eq!(progress.progress_percent, 75); // Executing
        assert_eq!(progress.current_step, "Executing transaction");

        intent.update_status(super::IntentStatus::Success);
        let progress = IntentProgressResponse::from_domain(&intent);
    ...
    fn test_fees_response() {
        ...
        let response = IntentFeesResponse::from_domain(&fees);
    ...
    fn test_metadata_response_filtering() {
        ...
        let response = IntentMetadataResponse::from_domain(&intent.metadata);
    ...
    fn test_status_to_string() {
        assert_eq!(super::IntentStatus::Pending.to_string(), "pending");
        assert_eq!(super::IntentStatus::Executing.to_string(), "executing");
        assert_eq!(super::IntentStatus::Success.to_string(), "success");
        assert_eq!(super::IntentStatus::Failed.to_string(), "failed");
    }

    #[test]
    fn test_next_steps_generation() {
        let steps_pending = IntentsResponse::generate_next_steps(&super::IntentStatus::Pending);
        assert!(steps_pending.iter().any(|s| s.contains("Monitor")));

        let steps_success = IntentsResponse::generate_next_steps(&super::IntentStatus::Success);
        assert!(steps_success.iter().any(|s| s.contains("wallet")));

        let steps_failed = IntentsResponse::generate_next_steps(&super::IntentStatus::Failed);
    ...
    fn test_execution_response() {
        ...
        let response = IntentExecutionResponse::from_domain(&execution_result);
    ...
    fn test_result_response() {
        ...
        let response = IntentResultResponse::from_domain(&execution_result);
