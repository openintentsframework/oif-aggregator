...
/// Validation errors for intent submissions
#[derive(Error, Debug)]
pub enum IntentValidationError {
    #[error("Invalid intent ID: {intent_id}")]
    InvalidIntentId { intent_id: String },
    ...
    #[error("Intent amount too small: {amount} (minimum {min_amount})")]
    AmountTooSmall { amount: String, min_amount: String },

    #[error("Intent amount too large: {amount} (maximum {max_amount})")]
...
pub enum IntentError {
    ...
    #[error("Intent already exists: {intent_id}")]
    AlreadyExists { intent_id: String },

    #[error("Intent is in invalid state: {intent_id} - current state: {current_state}")]
    InvalidState {
        intent_id: String,
        current_state: String,
    },

    #[error("Intent submission failed: {intent_id} - {reason}")]
    SubmissionFailed { intent_id: String, reason: String },

    #[error("Intent execution failed: {intent_id} - {reason}")]
    ExecutionFailed { intent_id: String, reason: String },

    #[error("Intent timeout: {intent_id} after {timeout_ms}ms")]
    Timeout { intent_id: String, timeout_ms: u64 },

    #[error("Intent cancelled: {intent_id} - {reason}")]
...
/// Intent execution result with detailed status
...
/// Intent simulation result
#[derive(Debug, Clone, PartialEq)]
pub struct IntentSimulationResult {
...
/// Intent health check for system monitoring
#[derive(Debug, Clone, PartialEq)]
pub struct IntentSystemHealth {
    pub is_healthy: bool,
    pub pending_intents: u64,
    pub executing_intents: u64,
    pub failed_intents_last_hour: u64,
...
impl IntentSystemHealth {
    pub fn healthy(
        pending_intents: u64,
        executing_intents: u64,
        success_rate: f64,
...
/// Intent rate limiting information
#[derive(Debug, Clone, PartialEq)]
pub struct IntentRateLimit {
